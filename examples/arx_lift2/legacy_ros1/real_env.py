...
from examples.aloha_arx_lift_real import robot_utils
...
def robot_action(ros_operator, args, action):
    ...
    left_action = action[:gripper_idx[0] + 1].copy()  # 取7维度
    # if gripper_gate != -1:
    #     left_action[gripper_idx[0]] = apply_gripper_gate(left_action[gripper_idx[0]], gripper_gate)
    left_action[gripper_idx[0]] = constants.PUPPET_GRIPPER_POSITION_UNNORMALIZE_FN(left_action[gripper_idx[0]])
    ...
    right_action[gripper_idx[0]] = constants.PUPPET_GRIPPER_POSITION_UNNORMALIZE_FN(right_action[gripper_idx[0]])
